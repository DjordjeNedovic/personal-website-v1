...
export default function ProjectPage({
  ...
  return (
    <Layout>
      ...
      <article className="px-6 md:px-0" data-sb-object-id={`projects/${slug}.mdx`}>
        <header>
...
export const getStaticProps = async ({ params }) => {
  ...
  const { mdxSource, data } = await getProjectsBySlug(params.slug);
  const prevPost = getPreviousProjectBySlug(params.slug);
  const nextPost = getNextProjectBySlug(params.slug);
...
export const getStaticPaths = async () => {
  const paths = getProjectsFilePaths()
