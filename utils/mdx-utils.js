...
// getPostFilePaths is the list of all mdx files inside the POSTS_PATH directory
export const getProjectsFilePaths = () => {
...
export const getProjects = () => {
  let posts = getProjectsFilePaths().map((filePath) => {
  ...
  posts = sortProjectsByDate(posts);

  return posts;
};

export const getProjectsBySlug = async (slug) => {
  const postFilePath = path.join(PROJECTS_PATH, `${slug}.mdx`);
  const source = fs.readFileSync(postFilePath);
